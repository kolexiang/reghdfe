...
mata:
	void function testit(struct MapProblem scalar S) {
		...
		//1337
		// S.fes[2].x
		//map_project(S, 1, st_data(., tokens("price u") ))
	}
end
...
//areg price ibn.trunk, absorb(turn)
areg trend ibn.trunk ibn.foreign, absorb(turn)
predict double xb, xbd
predict double resid, resid

//reg u rf##c.(gear_ratio displacement)
...
* S = map_init(transform={Kaczmarz,Cimmino,Symmetric_kaczmarz} , acceleration=...lol, tol=..., fe_varlist=...?)
* project(series, g, S)
* transform(series, S)
* .. = map(series, S)
* 
